...
public class DAO {
	...
	public static void joanEtorrikoaGordeDB(Agentzia ag, Bidai bidaia, Ekitaldi eki) {
		...
		try {
			konn.konexioaIreki();
			String sql = "insert into ekitaldiak values ( '" + eki.getEkikode() + "' , '" + "JoanekoHegaldia" + "' , '"
					+ eki.getBidaikode() + "')";
			preparedStatement = konn.konektatuta.prepareStatement(sql);
			preparedStatement.executeUpdate(sql);
			sql = "SELECT max(id_ekitaldia) from ekitaldiak;";
			...
			while (result.next()) {
				id = result.getInt(1);
				sql = "insert into joaneko_hegaldia values ( " + id + " , '" + eki.getHkode() + "' , '"
						+ eki.getIrtdata() + "' , '" + eki.getIrtordu() + "' , '" + eki.getIraupena() + "' , '"
						+ eki.getHprezio() + "' , '" + eki.getJatairportkod() + "' , '" + eki.getHelairportkod()
						+ "' , '" + eki.getAirelinea() + "')";
			...
			sql = "insert into ekitaldiak values ( '" + eki.getEkikode() + "' , '" + "JoanEtorrikoHegaldia" + "' , '"
					+ eki.getBidaikode() + "')";
			preparedStatement = konn.konektatuta.prepareStatement(sql);
			preparedStatement.executeUpdate(sql);

			sql = "insert into joan_etorriko_hegaldia values ( " + id + " , '" + eki.getHkode() + "' , '"
					+ eki.getBirtdata() + "' , '" + eki.getBirtordu() + "' , '" + eki.getBiraupena() + "' , '"
					+ eki.getBairelinea() + "')";
			preparedStatement = konn.konektatuta.prepareStatement(sql);
			preparedStatement.executeUpdate(sql);
...
public static void ezabatuEkitaldi(Agentzia ag) {
	...
	public static int bidaiakZenbatu() {
		int count = 0;
		try {
			konn.konexioaIreki();

			sql = "SELECT * from bidaiak";
